...
//  WiFi.setDNS(dns1, dns2);
  ETH.config((uint32_t)0x00000000, (uint32_t)0x00000000, (uint32_t)0x00000000, dns1, dns2);
...
//  if (WiFi.beginAP(ssid, channel) != WL_AP_FAILED) {
  WiFi.enableAP(true);
  if (WiFi.begin((char *)ssid, NULL, (int32_t)channel) != WL_NO_SHIELD) {
    response[4] = 1;
  } else {
    response[4] = 0;
  }
...
//  if (WiFi.beginAP(ssid, pass, channel) != WL_AP_FAILED) {
  WiFi.enableAP(true);
  if (WiFi.begin(ssid, pass, channel) != WL_NO_SHIELD) {
    response[4] = 1;
  } else {
    response[4] = 0;
  }
...
//    result = udps[socket].begin();
    ...
    if (host[0] != '\0') {
      if (setCACert) {
        tlsClients[socket].setCACert(CACERT_BUF);
      }
      if (setCert && setPSK) {
...
//  if (WiFi.hostByName(host, resolvedHostname)) {
  ...
  if (WiFi.hostByName(host, (IPAddress &)resolvedHostname)) {
    response[4] = 1;
  } else {
    response[4] = 0;
  }
...
int setRootCert(const uint8_t command[], uint8_t response[]){
  ets_printf("*** Called setRootCert\n");

  memset(CACERT_BUF, 0x00, sizeof(CACERT_BUF));
  memcpy(CACERT_BUF, &command[5], sizeof(CACERT_BUF));
...
int setClientCert(const uint8_t command[], uint8_t response[]){
  ...
  memcpy(CERT_BUF, &command[5], sizeof(CERT_BUF));
...
int setCertKey(const uint8_t command[], uint8_t response[]){
  ...
  memcpy(PK_BUFF, &command[5], sizeof(PK_BUFF));
...
const CommandHandlerType commandHandlers[] = {
  ...
  // 0x30 -> 0x3f
  disconnect, NULL, getIdxRSSI, getIdxEnct, reqHostByName, getHostByName, startScanNetworks, getFwVersion, NULL, sendUDPdata, getRemoteData, getTime, getIdxBSSID, getIdxChannel, /*ping*/NULL, getSocket,

  // 0x40 -> 0x4f
  setClientCert, setCertKey, setRootCert, NULL, sendDataTcp, getDataBufTcp, insertDataBuf, NULL, NULL, NULL, wpa2EntSetIdentity, wpa2EntSetUsername, wpa2EntSetPassword, wpa2EntSetCACert, wpa2EntSetCertKey, wpa2EntEnable,

  // 0x50 -> 0x5f
  setPinMode, setDigitalWrite, /*setAnalogWrite*/NULL, getAdcValue, softReset,
};
